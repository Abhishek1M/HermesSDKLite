...
import channels.XTCPClientConnectionClosed;
...
import message.iso8583.XFieldError;
...
public class Utility
{
     ...
     * @return
     */
    public static Hashtable<String, String> parsePipedString(String inputdata)
    {
        int startpos = 0;
        int pos;
        String temp;
        String[] arr;
        Hashtable<String, String> parsedData = new Hashtable<String, String>();
     ...
     * @return
     */
    public static HashMap<String, String> parseResponseString(String inputdata)
    {
        int startpos = 0;
        int pos;
        String temp;
     ...
     * @param clearKey
     * @return
     * @throws Exception public static String encryptECB(String inpBytes, String
     * clearKey) throws Exception { String algorithm = "DESede"; String
     * transformation = "DESede/ECB/PKCS5Padding"; byte[] keyvalue =
     * Transform.getData(Transform.fromHexToBin(clearKey)); DESedeKeySpec
     * keySpec = new DESedeKeySpec(keyvalue); SecretKey key =
     * SecretKeyFactory.getInstance(algorithm).generateSecret( keySpec); Cipher
     * encrypter = Cipher.getInstance(transformation);
     * encrypter.init(Cipher.ENCRYPT_MODE, key); byte[] finalCipher =
     * encrypter.doFinal(inpBytes.getBytes());
     *
     * return Transform.fromBinToHex(Transform.getString(finalCipher)); }
     */
    /**
     *
     * @param encKey
     * @return
     * @throws Exception public static String decryptDEK(String encKey) throws
     * Exception { String algorithm = "DESede"; String transformation =
     * "DESede/ECB/NoPadding"; String clearKey =
     * "33DD44812463A03A1EFAED7D535F356933DD44812463A03A1EFAED7D535F3569" ;
     * byte[] keyvalue = Transform.getData(Transform.fromHexToBin(clearKey));
     *
     * DESedeKeySpec keySpec = new DESedeKeySpec(keyvalue); SecretKey key =
     * SecretKeyFactory.getInstance(algorithm).generateSecret( keySpec);
     *
     * Cipher decrypter = Cipher.getInstance(transformation);
     * decrypter.init(Cipher.DECRYPT_MODE, key); byte[] finalCipher =
     * decrypter.doFinal(Transform.getData(Transform.fromHexToBin(encKey )));
     *
     * return Transform.fromBinToHex(Transform.getString(finalCipher)); }
     */
    /**
     ...
     */
    public static String encryptCard(String cardnumber) throws KeyStoreException,
            FileNotFoundException, IOException, NoSuchAlgorithmException,
            CertificateException
    {
        KeyStore ks = KeyStore.getInstance("JCEKS");
        ks.load(new FileInputStream(
                "D:\\Abhishek\\New Product Development\\Middleware\\keystore\\crypto.jks"),
                "prizm#12345".toCharArray());
     ...
     * @throws IOException
     */
    public static void sendINBCXDataWith2ByteHeader(INonBlockingConnection inbcx,
     ...
     * @throws IOException
     */
    public static void sendBCXDataWith2ByteHeader(IBlockingConnection inbcx,
            byte[] data) throws IOException
    {
        byte[] newdata = new byte[data.length + 2];
        byte[] len_byte;
     ...
     * @throws IOException
     */
    public static void sendINBCXDataWithDelimiter(INonBlockingConnection inbcx,
            String data) throws IOException
    {
        data = data + "\r\n";

        inbcx.write(Translate.getData(data));
    }
     ...
     * @throws IOException
     */
    public static void sendBCXDataWithDelimiter(IBlockingConnection ibcx,
            String data) throws IOException
    {
        data = data + "\r\n";

        ibcx.write(Translate.getData(data));
    }
     ...
     */
    public static int validateSufficientDatasizeByShortLengthField(
            INonBlockingConnection connection)
            throws IOException, BufferUnderflowException
    {
        connection.resetToReadMark();
        connection.markReadPosition();
        int length = connection.readShort();

        if (connection.available() < length)
        {
            System.out.println((new StringBuilder()).append("[").append(
                    connection.
                            getId()).append("]insufficient data. require ").append(
                    length).append(" got ").append(connection.available()).
                    toString());
     ...
     *
     * @param connection
     * @param removeLengthField
     * @return
     * @throws IOException
     * @throws BufferUnderflowException
     */
    public static int validateSufficientDatasizeByShortLengthField(
            INonBlockingConnection connection, boolean removeLengthField)
            throws IOException, BufferUnderflowException
    {
        connection.resetToReadMark();
        connection.markReadPosition();
     ...
     * @throws BufferUnderflowException
     */
    public static int validateSufficientDatasizeByShortLengthFieldBinary(
            INonBlockingConnection connection)
            throws IOException, BufferUnderflowException
    {
        connection.resetToReadMark();
        ...
        length = Integer.parseInt(Integer.toHexString(length));
     ...
     * @throws BufferUnderflowException
     */
    public static int validateSufficientDatasizeByIntLengthField(
            INonBlockingConnection connection)
            throws IOException, BufferUnderflowException
    {
        connection.resetToReadMark();
     ...
     * @throws BufferUnderflowException
     */
    public static int validateSufficientDatasizeForVisaByIntLengthField(
            INonBlockingConnection connection)
            throws IOException, BufferUnderflowException
    {
        connection.resetToReadMark();
    ...
    public static String sendAndRxHTTPMsg(String connurl, int timeout, String request)
    {
        ...
        try
        {
            ...
            DataOutputStream dos = new DataOutputStream(uconn.getOutputStream());
            dos.writeBytes(request);
            dos.flush();

            DataInputStream dis = new DataInputStream(uconn.getInputStream());
    ...
    {
        if (ihe == null || !ihe.getConnection().isOpen())
        {
            return false;
        }
    ...
    public static byte[] sendAndReceive2ByteHeaderData(String ip, int port, byte[] data, int timeout)
    {
        ...
        try
        {
            ...
            // Calculate the 2 byte TCP/IP header
            len_byte = Translate.getData(Translate.fromHexToBin(Utility.resize(
                    Integer.toHexString(data_len), 4, "0", false)));

            // Form the new data packet which is inclusive of the 
            System.arraycopy(len_byte, 0, newdata, 0, len_byte.length);
            System.arraycopy(data, 0, newdata, 2, data.length);
     ...
     * @param inbcx
     * @throws IOException
     * @throws XTCPClientConnectionClosed
     * @throws XFieldError
     */
    public static void processAction(Action action, INonBlockingConnection inbcx) throws IOException,
            XTCPClientConnectionClosed, XFieldError
    {
        boolean flag = true;
        ...
        while (flag)
        {
            byte[] data_r = (byte[]) action.getDataToRemote();

            if (data_r == null)
            {
                flag = false;
            } else if (inbcx.isOpen())
            {
                // sends message to remote
                sendINBCXDataWith2ByteHeader(inbcx, data_r);
            }
        }
    }
     ...
     * @throws XFieldError
     */
    public static void processActionDL(Action action, INonBlockingConnection inbcx) throws IOException,
            XTCPClientConnectionClosed, XFieldError
    {
        boolean flag = true;
        ...
        while (flag)
        {
            ...
            {
                // sends message to remote
                sendINBCXDataWithDelimiter(inbcx, Translate.getString(data_r));
            }
        }
    }
     ...
     * @throws XFieldError
     */
    public static void processActionContext(Action action, ChannelHandlerContext ctx) throws IOException,
            XTCPClientConnectionClosed, XFieldError
    {
        boolean flag = true;
        ...
        while (flag)
        {
            ...
            {
                // sends message to remote
                ByteBuf rsp = PooledByteBufAllocator.DEFAULT.buffer(data_r.length);
                rsp.writeBytes(data_r);
     ...
     * @return
     */
    public static boolean updateSelfStatus(String interfaceName, Connection conn)
    {
    	Timestamp ts;
    	...
    	try
    	{
            ...
            ps = conn.prepareStatement(
                    "update interface set last_update_datetime = ? where pname = ?");
