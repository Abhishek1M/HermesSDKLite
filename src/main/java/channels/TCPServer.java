...
public class TCPServer
{
     ...
     */
    public TCPServer(String name, String ipaddress, int port, AppQueue event_q,
...
//                    backlog);
            ...
            IDisconnectHandler, IConnectionTimeoutHandler, IIdleTimeoutHandler
    {
        ...
        {
            int available_bytes = inbc.available();

            if (available_bytes > 0)
            {
                //byte[] data;
                byte[] parseddata;
                int utflen = inbc.readShort();

                //data = inbc.readBytesByLength(available_bytes);
                parseddata = inbc.readBytesByLength(utflen);

                //parseddata = new byte[data.length - 2];
                //parseddata = new byte[data.length];

                //System.arraycopy(data, 2, parseddata, 0, data.length - 2);
                //System.arraycopy(data, 2, parseddata, 0, data.length - 2);
