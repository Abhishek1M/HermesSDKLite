...
public class TCPClientVisa
{
     ...
     */
    public TCPClientVisa(INonBlockingConnection inbcx)
    {
        this.inbcx = inbcx;
    }
     ...
     */
    public TCPClientVisa(String name, AppQueue event_q, String remoteaddress, int port,
    ...
    public boolean reconnect(long timer)
    {
        ...
        {
            Logger.getLogger(TCPClientVisa.class.getName()).
                    log(Level.SEVERE, "InterruptedException#reconnect", ex);
        }
        ...
        {
            Logger.getLogger(TCPClientVisa.class.getName()).
                    log(Level.SEVERE, "IOException#reconnect", ex);
            return false;
        }
    }
     ...
     * channel is closed and data transmission is tried
     */
    public void senddatawith4bytehdr(byte[] data) throws IOException,
            XTCPClientConnectionClosed
    {
        ...
        len_byte = Translate.getData(Translate.fromHexToBin(Utility.resize(Integer.
                toHexString(data_len), 4, "0", false)) + Translate.fromHexToBin("0000"));
            ...
            IIdleTimeoutHandler, IConnectExceptionHandler
    {
        ...
        public boolean onData(INonBlockingConnection inbc) throws IOException
        {
            try
            {
                ...
                int utflen = Utility.
                        validateSufficientDatasizeForVisaByIntLengthField(inbc);

                if (utflen > 0)
                {
                    ByteBuffer[] bbuf = inbc.readByteBufferByLength(utflen);
                    tempparsedata = DataConverter.toBytes(bbuf);

                    parseddata = new byte[tempparsedata.length - 2];
                    System.arraycopy(tempparsedata, 2, parseddata, 0, tempparsedata.length - 2);
